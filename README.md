...
### 3. Run V2-rtl test (alpha_mm strategy alone)

Verifies golden math == cycle pymodel == RTL bit-exact:
...
The framework establishes the L1 → L2 → L3 → V4 → real-data evidence chain:

| Layer | Reference | Test | Result |
|---|---|---|---|
| L1 (math golden) | `hftbacktest 2.3.0` ROIVectorMarketDepth | `tb/alpha_mm/test_alpha_mm_rtl_eq_golden` | 0 mismatches |
| L2 (cycle pymodel) | == L3 (RTL) lockstep | `tb/alpha_mm/test_alpha_mm_rtl_eq_pymodel` | 0 mismatches (200 cases) |
| L3 (RTL pipeline) | real NASDAQ MSFT | `tb/hft_alpha_perf` | 5-cycle pipeline, 81% accept |
| V4 (P&R) | Alveo U50, 333 MHz target | `synth/run_v4.tcl` | +0.370 ns slack |
